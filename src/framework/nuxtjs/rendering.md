...
示意图：
传统客户端渲染 (CSR)：
[浏览器请求] → [服务器返回空HTML] → [下载JS] → [执行Vue.js] → [渲染页面]
...
示意图：
[浏览器请求] → [服务器运行Vue.js生成HTML] → [返回完整HTML]
        ↓
[用户立即看到内容] → [后台加载JS] → [Vue.js接管交互]
...
示意图：
[浏览器请求] → [服务器返回空HTML和JS] → [下载JS] → [执行Vue.js] → [渲染页面]
